...
The SO-100 leader arm supports optional **gripper force feedback** — when the follower grasps an object, the leader gripper resists squeezing past the follower's achieved grasp position (virtual wall effect). See the [SO-101 force feedback docs](./so101#gripper-force-feedback) for configuration details and parameters.

> [!NOTE]
> Unlike the leader arm motors, the gripper motor **must retain its gears** for force feedback to work — the gear train transmits the virtual wall resistance to the user's hand.
